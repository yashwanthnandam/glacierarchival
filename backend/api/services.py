...
class S3Service:
    ...
    def check_rate_limits(self, operation_type='upload', file_size_bytes=0):
        ...
        if operation_type == 'upload':
            ...
            if plan.free_retrieval_gb > 0 and user_plan.retrieval_remaining_gb < file_size_gb:
                ...
                if False:  # Disabled additional cost logic
                    # In a real implementation, you might want to charge the user
                    print(f"Retrieval will incur additional cost: ₹{additional_cost:.2f}")
    ...
    def multipart_upload(self, file, s3_key):
    ...
    def create_batch_delete_manifest(self, s3_keys, manifest_s3_key):
        """Create a manifest file for AWS S3 Batch Operations"""
        import json
        ...
        manifest_content = '\n'.join([json.dumps(line) for line in manifest_lines])
        
        # Upload manifest to S3
        from io import BytesIO
        manifest_bytes = BytesIO(manifest_content.encode('utf-8'))
        ...
        return f"s3://{self.s3_config.bucket_name}/{manifest_s3_key}"

    def create_batch_delete_job(self, s3_keys, job_id=None):
        """Create AWS S3 Batch Delete Job with safety measures"""
        if not job_id:
            job_id = f"bulk-delete-{self.user.id}-{int(timezone.now().timestamp())}"
        
        # Safety check: Limit batch size
        if len(s3_keys) > 1000:
            raise ValueError("Batch size too large. Maximum 1000 objects per batch.")
        
        if len(s3_keys) < 2:
            raise ValueError("Batch operations require at least 2 objects. Use individual delete for single objects.")
        
        try:
            # Create manifest file
            manifest_key = f"batch-manifests/{job_id}-manifest.json"
            manifest_location = self.create_batch_delete_manifest(s3_keys, manifest_key)
            
            # Create completion report location
            completion_report_key = f"batch-reports/{job_id}-completion-report.json"
            
            # Create batch job
            response = self.s3_client.create_job(
                AccountId=self._get_account_id(),
                ConfirmationRequired=False,  # Set to True for production safety
                Operation={
                    'S3DeleteObject': {
                        'Bucket': self.s3_config.bucket_name
                    }
                ...
                Manifest={
                    ...
                    'Location': {
                        ...
                        'ETag': self._get_object_etag(manifest_key)
                    }
                },
                Priority=10,
                RoleArn=self._get_batch_operations_role_arn(),
                ClientRequestToken=job_id,
                Report={
                    'Enabled': True,
                    'Bucket': f"s3://{self.s3_config.bucket_name}",
                    'Prefix': f"batch-reports/{job_id}",
            ...
            return {
                ...
                'completion_report_key': completion_report_key,
                'object_count': len(s3_keys)
            }
            
        except Exception as e:
            # Clean up manifest file on failure
            try:
                self.s3_client.delete_object(
                    Bucket=self.s3_config.bucket_name,
                    Key=manifest_key
                ...
                pass
            raise Exception(f"Failed to create batch delete job: {str(e)}")

    def get_batch_job_status(self, job_id):
        """Get status of AWS S3 Batch Job"""
        try:
            response = self.s3_client.describe_job(
                AccountId=self._get_account_id(),
                JobId=job_id
            )
            ...
            return {
                ...
                'progress_summary': response['Job'].get('ProgressSummary', {}),
                'creation_date': response['Job']['CreationDate'],
                'completion_date': response['Job'].get('CompletionDate'),
                'failure_reason': response['Job'].get('FailureReasons', [])
            }
        except Exception as e:
    ...
    def _get_object_etag(self, key):
        ...
        try:
            ...
            raise Exception(f"Failed to get object ETag: {str(e)}")

    def _get_batch_operations_role_arn(self):
        """Get or create IAM role for batch operations"""
        # In production, you should create a dedicated IAM role
        # For now, we'll use a placeholder that should be configured
        role_name = f"GlacierArchival-BatchOperations-{self.user.id}"
        return f"arn:aws:iam::{self._get_account_id()}:role/{role_name}"

    def safe_bulk_delete_with_batch_operations(self, s3_keys, use_batch=True):
        """Safely delete multiple files using AWS Batch Operations or fallback to individual deletes"""
        
        # Safety checks
        if not s3_keys:
            return {'deleted': 0, 'method': 'none', 'message': 'No files to delete'}
        
        if len(s3_keys) > 1000:
            raise ValueError("Too many files for batch operation. Maximum 1000 files.")
        
        # For small batches, use individual deletes (safer)
        if len(s3_keys) < 10 or not use_batch:
            return self._individual_bulk_delete(s3_keys)
        
        try:
            # Try AWS Batch Operations for larger batches
            batch_result = self.create_batch_delete_job(s3_keys)
            ...
            return {
                ...
                'message': f'Batch delete job created for {len(s3_keys)} files'
            }
            
        except Exception as e:
            print(f"Batch operations failed, falling back to individual deletes: {str(e)}")
            # Fallback to individual deletes
            return self._individual_bulk_delete(s3_keys)

    def _individual_bulk_delete(self, s3_keys):
        """Fallback method: delete files individually"""
        deleted_count = 0
        failed_deletions = []
        ...
        return {
            ...
            'message': f'Deleted {deleted_count} files individually'
        }
...
class MediaFileService(BaseService):
    ...
    def archive_file(self, media_file):
        ...
        # Determine AWS storage class based on plan
        aws_storage_class = self._get_aws_storage_class(user_plan.plan.aws_storage_type)
    ...
    def restore_file_legacy(self, media_file, restore_tier='Standard'):
        """Restore an archived file with hibernation plan limits and cost tracking"""
        if media_file.status != 'archived':
            raise ValueError("File is not archived")
        ...
        user_plan = self.hibernation_service.get_user_plan()
        if not user_plan:
            raise ValueError("Active hibernation plan required for file restoration")
        
        # Calculate retrieval cost
        retrieval_cost = self.hibernation_service.calculate_retrieval_cost(media_file.file_size)
        ...
        try:
            # Calculate estimated completion time based on restore tier
            restore_hours = RESTORE_TIME_ESTIMATES.get(restore_tier, 4)
            estimated_completion = timezone.now() + timedelta(hours=restore_hours)
            ...
            # In production, this would initiate a real Glacier restore
            self._simulate_glacier_restore(job, restore_tier)
    ...
    def _simulate_glacier_restore(self, job, restore_tier):
        """Simulate Glacier restore process with realistic timing"""
        import threading
        import time
        ...
        def restore_worker():
            try:
                ...
                progress_steps = [25, 50, 75, 90, 100]
                step_duration = restore_seconds / len(progress_steps)
                ...
                for i, progress in enumerate(progress_steps):
                    ...
                    if progress == 100:
                        ...
                        restore_key = f"uploads/{self.user.username}/{job.media_file.filename}"
                        
                        # Copy file back to uploads (simulating restore)
                        self.s3_service.copy_file(job.media_file.s3_key, restore_key)
                        
                        # Update media file
                        job.media_file.status = 'restored'
                        job.media_file.s3_key = restore_key
                        job.media_file.save()
        ...
        thread = threading.Thread(target=restore_worker)
        thread.daemon = True
        thread.start()
